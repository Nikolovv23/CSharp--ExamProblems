// On the first three lines, you will receive the quantity of food, hay, and cover, which Merry buys for a month (30 days). On the fourth line,
// you will receive the guinea pig's weight.
// Every day Puppy eats 300 gr of food. Every second day Merry first feeds the pet, then gives it a certain amount of hay equal to 5% of the rest of the food.
// On every third day, Merry puts Puppy cover with a quantity of 1/3 of its weight.
// Calculate whether the quantity of food, hay, and cover, will be enough for a month.
// If Merry runs out of food, hay, or cover, stop the program!
// Input
// •	On the first line – quantity food in kilograms - a floating-point number in the range [0.0 – 10000.0].
// •	On the second line – quantity hay in kilograms - a floating-point number in the range [0.0 – 10000.0].
// •	On the third line – quantity cover in kilograms - a floating-point number in the range [0.0 – 10000.0].
// •	On the fourth line – guinea's weight in kilograms - a floating-point number in the range [0.0 – 10000.0].
// Output
// •	If the food, the hay, and the cover are enough, print:
// o   "Everything is fine! Puppy is happy! Food: {excessFood}, Hay: {excessHay}, Cover: {excessCover}."
// •	If one of the things is not enough, print:
// o   "Merry must go to the pet store!"
// The output values must be formatted to the second decimal place!

//     Input:
// 10
// 5
// 5.2
// 1
//     Output:
// Everything is fine! Puppy is happy! Food: 1.00, Hay: 1.10, Cover: 1.87.


namespace GuineaPig
{
    internal class Program
    {
        static void Main(string[] args)
        {

            double quantityFood = double.Parse(Console.ReadLine());
            double quantityHay = double.Parse(Console.ReadLine());
            double quantityCover = double.Parse(Console.ReadLine());
            double guineasWeight = double.Parse(Console.ReadLine());

            quantityFood *= 1000;
            quantityHay *= 1000;
            quantityCover *= 1000;
            guineasWeight *= 1000;

            for (int i = 1; i <= 30; i++)
            {
                quantityFood -= 300;
                if (quantityFood < 0)
                {
                    Console.WriteLine("Merry must go to the pet store!");
                    return;
                }
                //guineasWeight += 300;

                if (i % 2 == 0)
                {
                    quantityHay -= 0.05 * quantityFood;
                    if (quantityHay < 0)
                    {
                        Console.WriteLine("Merry must go to the pet store!");
                        return;
                    }
                    //guineasWeight += 0.05 * quantityFood;
                }

                if (i % 3 == 0)
                {
                    quantityCover -= Math.Round((guineasWeight / 3), 2);

                    if (quantityCover < 0)
                    {
                        Console.WriteLine("Merry must go to the pet store!");
                        return;
                    }
                }
            }
            quantityFood /= 1000;
            quantityHay /= 1000;
            quantityCover /= 1000;

            Console.WriteLine($"Everything is fine! Puppy is happy! Food: {quantityFood:f2}, Hay: {quantityHay:f2}, Cover: {quantityCover:f2}.");
        }
    }
}
